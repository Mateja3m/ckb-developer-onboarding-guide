...
- you are only reading chain data and do not need local process management, local chain state, or devnet behavior

This guide documents a validated public RPC path in [Quick Start](../quick-start.md). Use the local node path only when you need local process control, local logs, or devnet behavior.
...
This guide focuses on the repository-validated OffCKB onboarding path.
Broader installation variants, alternate networks, and production node operation remain outside this beginner guide and should be checked against official CKB documentation.
...
This page documents the first repository-validated onboarding path. It does not replace official CKB installation references for broader setups, alternate networks, or production-oriented node operation.
