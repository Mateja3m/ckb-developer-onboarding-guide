...
This repository currently contains the Week 1 foundation for Milestone 1:
...
The rest of the documentation is intentionally scaffolded as placeholders so the guide can expand in a structured way without pretending unfinished sections are already validated.
...
- Exact CKB-specific commands, outputs, and configuration details are deferred until they are tested.
- Any step that still requires confirmation is marked as `TODO: VALIDATE`.
...
2. Use the remaining documents as structure, not as finished instructions.

   The placeholder files show how the complete guide will be organized, but they are not yet validated onboarding steps.
...
4. Use the documentation map to understand the current scope.
   ...
   - [03 Quick Start](docs/03-quick-start.md): placeholder
   - [04 CKB Node Setup](docs/04-ckb-node-setup.md): placeholder
   - [05 RPC Setup](docs/05-rpc-setup.md): placeholder
   - [06 Indexer Setup](docs/06-indexer-setup.md): placeholder
   - [07 Configuration Setup](docs/07-configuration-setup.md): placeholder
...
- only the first four files are drafted for Week 1
- the remaining files are placeholders by design
...
If a step or recommendation is marked with `TODO: VALIDATE`, it still needs confirmation before being treated as final documentation.
